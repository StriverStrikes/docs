...
When calling the `/join` API, include a `mediaStreaming` object in the payload. For the exact payload structure, refer to the [API Payload Details](http://localhost:3000/rest/sending-a-agent#param-media-streaming). This object should contain the following fields:
...
- **`speakerTimeline`**: Provides speaker attribution. If no one is in the meeting, this will be an empty array. If there are no active speakers, the `audioChunk` will contain silence.
