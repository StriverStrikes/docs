...
<Tabs>
  ...
  ```bash
  curl --location 'http://your-company-domain/api/v1/agent/join' \
  ...
  --data '{
      ...
      "webhookUrl": "https://your-company-domain/bot/events"
  }
  ```
  ...
  myHeaders.append("Authorization", `Token ${YOUR_API_KEY}`);
  ...
  fetch("http://your-company-domain/api/v1/agent/join", requestOptions)
  ...
  url = "http://your-company-domain/api/v1/agent/join"
...
<ResponseField name="transcriptionProvider" type="object" optional>
  ...
  **Properties**:
  - **name**: One of:
    ...
    - `"deepgram"` or `"rev"`: Requires API key
  - **apiKey**: Required when using deepgram or rev providers
